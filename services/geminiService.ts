...
const responseCache = new Map<
  string,
  { arabic: string; translation: string; category: string }
>();
...
const DUA_RESPONSE_SCHEMA = {
  ...
  properties: {
    arabic: {
      type: Type.STRING,
      description:
        "The extracted Arabic text. Include all diacritics (harakat) if visible.",
    },
    translation: {
      type: Type.STRING,
      description:
        "A poetic and faithful English translation of the spiritual meaning.",
    },
    category: {
      type: Type.STRING,
...
const OCR_ARABIC_CLEANUP_SCHEMA = {
  ...
  properties: {
    arabic: {
      type: Type.STRING,
      description:
        "Arabic text corrected from OCR artifacts with minimal edits.",
    },
  },
  required: ["arabic"],
};
...
const setCached = (
  key: string,
  value: { arabic: string; translation: string; category: string },
) => {
...
const assertGeminiConfigured = () => {
  if (!GEMINI_API_KEY) {
    throw new Error(
      "Missing Gemini API key. Add `GEMINI_API_KEY` to `.env.local` and restart the app.",
    );
  }
};

export const processDuaFromImage = async (
  base64Image: string,
      ...
      const response = await withTimeout(
        ai.models.generateContent({
          ...
          config: {
            ...
            responseSchema: includeTranslation
              ? DUA_RESPONSE_SCHEMA
              : DUA_ARABIC_ONLY_RESPONSE_SCHEMA,
            temperature: 0.05,
...
export const cleanupArabicOcrText = async (arabicText: string) => {
  ...
  const response = await generateContentWithRetry(
    {
      model,
      contents: `Fix OCR mistakes in this Arabic text (do not translate):\n${cleaned}`,
      config: {
        responseMimeType: "application/json",
...
export const processDuaFromUrl = async (
  url: string,
  includeTranslation = true,
) => {
  assertGeminiConfigured();
  ...
  const response = await withTimeout(
    ai.models.generateContent({
      model,
      contents: `Extract main dua from URL: ${cleanedUrl}`,
      config: {
        responseMimeType: "application/json",
        responseSchema: includeTranslation
          ? DUA_RESPONSE_SCHEMA
